...
def _token_ngram_analyzer(text: str, min_n: int = 1, max_n: int = 6) -> list[str]:
    """
    Return contiguous token chunks for TF-IDF.

    This keeps order/structure: POWER-PANEL-BUS contributes POWER_PANEL,
    PANEL_BUS, POWER_PANEL_BUS, etc. Single tokens are retained only as a
    weak backstop for very short rules.
    """
    toks = [t for t in text.split(" ") if t]
    grams: list[str] = []
    for n in range(max(min_n, 1), min(max_n, len(toks)) + 1):
        grams.extend("__".join(toks[i:i + n]) for i in range(len(toks) - n + 1))
...
def _longest_common_contiguous_len(a: list[str], b: list[str]) -> int:
    """Length of the longest exact consecutive token block shared by a and b."""
    if not a or not b:
        return 0
    prev = [0] * (len(b) + 1)
    best = 0
...
def structural_similarity(new_toks: list[str], master_toks: list[str]) -> dict[str, float]:
    """
    Structure-aware rule similarity.

    The score favors exact consecutive chunks and suffix agreement. It does
    not treat the rule as an unordered bag of tokens.
    """
    denom = max(len(new_toks), len(master_toks), 1)
    lcs = _longest_common_contiguous_len(new_toks, master_toks)
    ...
    score = (
        ...
        + 0.15 * (prefix / max(len(new_toks), 1))
        + 0.10 * order_ratio
    )
    return {
        "score": round(float(score), 4),
        "longest_common_block": float(lcs),
        "prefix_match_len": float(prefix),
        "suffix_match_len": float(suffix),
        "ordered_ratio": round(float(order_ratio), 4),
    }


def build_suffix_index(master: pd.DataFrame) -> dict[tuple[str, ...], list[dict[str, object]]]:
    """Map every exact suffix tuple to master rules that end with that suffix."""
    suffix_index: dict[tuple[str, ...], list[dict[str, object]]] = defaultdict(list)
    for idx, (rule, toks, sev) in enumerate(zip(master["Rule"], master["tokens"], master["Severity"])):
        for start in range(len(toks)):
            suffix = tuple(toks[start:])
...
def suffix_chain_matches(
    tokens: list[str],
    suffix_index: dict[tuple[str, ...], list[dict[str, object]]],
    top_rules: int = 5,
) -> list[dict[str, object]]:
    """
    Exact suffix-chain lookup:
      a-b-c-d-e, then b-c-d-e, then c-d-e, then d-e, then e.
    """
    matches: list[dict[str, object]] = []
    for start in range(len(tokens)):
        suffix = tuple(tokens[start:])
        found = suffix_index.get(suffix, [])
        if not found:
            continue
        dist = Counter(row["Severity"] for row in found)
        sev, sev_count = dist.most_common(1)[0]
        matches.append({
            "match_suffix": "-".join(suffix),
            "match_len": len(suffix),
            "coverage": round(len(suffix) / max(len(tokens), 1), 3),
            "n_master_matches": len(found),
            "predicted_severity": sev,
            "confidence": round(sev_count / len(found), 3),
            "severity_distribution": dict(dist),
            "matched_rules": [
                f"{row['Severity']}|{row['Rule']}" for row in found[:top_rules]
            ],
        })
    return matches


def suffix_chain_predict(
    tokens: list[str],
    suffix_index: dict[tuple[str, ...], list[dict[str, object]]],
) -> dict[str, object]:
    matches = suffix_chain_matches(tokens, suffix_index)
    if not matches:
        return {
            "sev": None,
            "conf": 0.0,
    ...
    best = sorted(
        matches,
        key=lambda m: (m["match_len"], m["confidence"], m["n_master_matches"]),
        reverse=True,
    )[0]
    return {
        "sev": best["predicted_severity"],
        "conf": best["confidence"],
        "match": best["match_suffix"],
        "n_matches": best["n_master_matches"],
        "all_matches": matches,
    }


def knn_predict(master: pd.DataFrame, new_rules: pd.DataFrame, k: int = 5):
    """TF-IDF over contiguous token chunks + cosine kNN."""
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.neighbors import NearestNeighbors

    vec = TfidfVectorizer(analyzer=_token_ngram_analyzer, lowercase=False)
    ...
    master_toks = master["tokens"].to_numpy()
    for i in range(len(new_rules)):
        ...
        for s, w in zip(neigh_sevs, sims):
            weights_by_sev[s] += w
        total_weight = sum(weights_by_sev.values())
        if total_weight <= 1e-12:
            weighted_sev = Counter(neigh_sevs).most_common(1)[0][0]
            weighted_conf = 0.0
        else:
            weighted_sev = max(weights_by_sev, key=weights_by_sev.get)
            weighted_conf = weights_by_sev[weighted_sev] / total_weight
        neighbors = []
        for j in range(len(idxs[i])):
            m_idx = idxs[i][j]
            sim_details = structural_similarity(new_rules["tokens"].iloc[i], master_toks[m_idx])
            combined_sim = 0.55 * float(sims[j]) + 0.45 * sim_details["score"]
            neighbors.append({
                "rule": master_rules[m_idx],
                "severity": master_sevs[m_idx],
                "ngram_cosine": round(float(sims[j]), 3),
                "structural_score": sim_details["score"],
                "combined_similarity": round(float(combined_sim), 3),
                "longest_common_block": int(sim_details["longest_common_block"]),
                "prefix_match_len": int(sim_details["prefix_match_len"]),
                "suffix_match_len": int(sim_details["suffix_match_len"]),
            })
        neighbors.sort(key=lambda row: row["combined_similarity"], reverse=True)
...
def predict_all(master: pd.DataFrame, new_rules: pd.DataFrame) -> pd.DataFrame:
    ...
    suf_root = build_trie(master["tokens"].apply(lambda t: list(reversed(t))),
                          master["Severity"])
    suffix_index = build_suffix_index(master)
    ...
    for i, rule in enumerate(new_rules["Rule"]):
        ...
        chain = suffix_chain_predict(toks, suffix_index)
        k_sev, k_conf = knn[i]["sev"], knn[i]["conf"]

        # Ensemble: weighted vote. The exact suffix-chain match gets a
        # stronger vote because it implements the business rule directly.
        votes = Counter()
        for sev, conf, weight in [
            (p_sev, p_conf, 0.8),
            (s_sev, s_conf, 1.0),
            (chain["sev"], chain["conf"], 1.4),
            (k_sev, k_conf, 0.8),
        ]:
            if sev is not None:
                votes[sev] += conf * weight
        if votes:
            final_sev, final_score = votes.most_common(1)[0]
            vote_total = sum(votes.values())
            final_conf = final_score / vote_total if vote_total > 1e-12 else 0.0
        else:
            final_sev, final_conf = None, 0.0

        non_null_preds = [x for x in [p_sev, s_sev, chain["sev"], k_sev] if x is not None]
        predictors = set(non_null_preds)
        agree = len(predictors)
        if len(non_null_preds) >= 2 and agree == 1:
            method = "all_available_methods_agree"
        elif len(non_null_preds) == 1:
            method = "single_method"
        else:
            method = f"split_{agree}way"
        suffix_chain_summary = " || ".join(
            f"{m['match_suffix']} [{m['predicted_severity']} {m['confidence']:.0%}; n={m['n_master_matches']}]"
            for m in chain["all_matches"][:5]
        )
        similar_rules = " || ".join(
            f"{n['combined_similarity']:.3f}|{n['severity']}|{n['rule']}"
            for n in knn[i]["neighbors"][:5]
        )
        ...
        rows.append({
            ...
            "suffix_chain_pred": chain["sev"],
            "suffix_chain_conf": round(float(chain["conf"]), 3),
            "suffix_chain_match": chain["match"],
            "suffix_chain_n_master_matches": chain["n_matches"],
            "suffix_chain_all_matches": suffix_chain_summary,
            "knn_pred": k_sev, "knn_conf": round(k_conf, 3),
            "knn_top_neighbor": knn[i]["neighbors"][0]["rule"] if knn[i]["neighbors"] else "",
            "knn_top_sim": knn[i]["neighbors"][0]["combined_similarity"] if knn[i]["neighbors"] else 0.0,
            "most_similar_rules": similar_rules,
        })
...
# ---------------------------------------------------------------------------
# Closest-rule helpers  (for explain_predictions)
# ---------------------------------------------------------------------------
def _closest_prefix_rule(
    master: pd.DataFrame, new_toks: list[str], match_path: str
) -> "tuple[str, str]":
    """
    Among master rules that share `match_path` as a prefix, return the one
    that continues to match new_toks for the most additional tokens.
    Returns (rule_string, severity).
    """
    if not match_path:
        return "", ""
    path_toks = [t for t in match_path.split("-") if t]
    n = len(path_toks)

    best_rule, best_sev, best_shared = "", "", 0
    for rule, toks, sev in zip(master["Rule"], master["tokens"], master["Severity"]):
        if len(toks) >= n and toks[:n] == path_toks:
            extra = sum(
                1 for j in range(n, min(len(toks), len(new_toks)))
                if toks[j] == new_toks[j]
            )
            if (n + extra) > best_shared:
                best_shared = n + extra
                best_rule, best_sev = rule, sev
    return best_rule, best_sev


def _closest_suffix_rule(
    master: pd.DataFrame, new_toks: list[str], match_path_natural: str
) -> "tuple[str, str]":
    """
    Among master rules that share `match_path_natural` as a suffix (natural
    token order), return the one that continues to match backwards for the
    most additional tokens.
    Returns (rule_string, severity).
    """
    if not match_path_natural:
        return "", ""
    path_toks = [t for t in match_path_natural.split("-") if t]
    n = len(path_toks)
    ...
    for rule, toks, sev in zip(master["Rule"], master["tokens"], master["Severity"]):
        if len(toks) >= n and toks[-n:] == path_toks:
            extra = sum(
                1 for j in range(1, min(len(toks) - n, len(new_toks) - n) + 1)
                if toks[-(n + j)] == new_toks[-(n + j)]
            )
            if (n + extra) > best_shared:
                best_shared = n + extra
                best_rule, best_sev = rule, sev
...
def explain_predictions(
    ...
    """
    For every new rule, print a layman-friendly explanation of how each of
    the three prediction methods voted and which master rule it considered
    the closest match.
    """
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.neighbors import NearestNeighbors
    ...
    nn_model = NearestNeighbors(
        n_neighbors=min(top_k_neighbors, len(master)), metric="cosine"
    ).fit(X_master)
    dists, idxs = nn_model.kneighbors(X_new)

    master_sevs  = master["Severity"].to_numpy()
    master_rules_arr = master["Rule"].to_numpy()
    suffix_index = build_suffix_index(master)

    def _short(s: str, w: int = 74) -> str:
        return s if len(s) <= w else s[:w - 3] + "..."

    print("\n" + "=" * 70)
    print("PLAIN-ENGLISH PREDICTION EXPLANATIONS")
    print("=" * 70)

    for i, (rule, toks) in enumerate(zip(new_rules["Rule"], new_rules["tokens"])):

        # ── ① Prefix trie ────────────────────────────────────────────────
        p_sev, p_conf, p_path, p_dist = predict_with_trie(toks, pref_root)
        p_depth = len(p_path.split("-")) if p_path else 0
        p_n     = sum(p_dist.values()) if p_dist else 0
        p_best, p_best_sev = _closest_prefix_rule(master, toks, p_path)

        # ── ② Suffix trie ────────────────────────────────────────────────
        s_sev, s_conf, s_path_rev, s_dist = predict_with_trie(
            list(reversed(toks)), suf_root
        )
        s_path_nat = (
            "-".join(reversed(s_path_rev.split("-"))) if s_path_rev else ""
        )
        s_depth = len(s_path_nat.split("-")) if s_path_nat else 0
        s_n     = sum(s_dist.values()) if s_dist else 0
        s_best, s_best_sev = _closest_suffix_rule(master, toks, s_path_nat)

        # ── ②b Exact suffix-chain lookup ────────────────────────────────
        chain = suffix_chain_predict(toks, suffix_index)

        # ── ③ kNN ────────────────────────────────────────────────────────
        neigh_sevs  = master_sevs[idxs[i]]
        neigh_rules = master_rules_arr[idxs[i]]
        sims        = 1 - dists[i]
        weights: dict[str, float] = defaultdict(float)
        for sv, w in zip(neigh_sevs, sims):
            weights[sv] += w
        if sum(weights.values()) <= 1e-12:
            knn_sev = Counter(neigh_sevs).most_common(1)[0][0]
            knn_conf = 0.0
        else:
            knn_sev = max(weights, key=weights.get)
            knn_conf = weights[knn_sev] / sum(weights.values())
        knn_ctr  = Counter(neigh_sevs)
        ...
        for sv, cf, weight in [
            ...
            (s_sev, s_conf, 1.0),
            (chain["sev"], chain["conf"], 1.4),
            (knn_sev, knn_conf, 0.8),
        ]:
            if sv:
                votes[sv] += cf * weight
        final_sev  = votes.most_common(1)[0][0] if votes else "Unknown"
        final_conf = (
            votes.most_common(1)[0][1] / sum(votes.values())
            if votes and sum(votes.values()) > 1e-12 else 0.0
        )
        available = [x for x in [p_sev, s_sev, chain["sev"], knn_sev] if x is not None]
        all_agree = len(available) >= 2 and len(set(available)) == 1

        # ── Header ───────────────────────────────────────────────────────
        print(f"\n{'━' * 70}")
        print(f"RULE: {_short(rule)}")
        print(f"{'━' * 70}")

        col = _CSEV.get(final_sev, "")
        agree_tag = "✓ available methods agree" if all_agree else "⚠ methods disagree — review recommended"
        print(f"  VERDICT  →  {col}{final_sev}{_RST}  ({final_conf:.0%} confidence)  {agree_tag}")

        # ── ① Start-of-rule match ─────────────────────────────────────────
        c1 = _CSEV.get(p_sev, "")
        print(f"\n  ① START-OF-RULE MATCH  →  {c1}{p_sev or 'no match'}{_RST}  [{p_conf:.0%}]")
        if p_path:
            dist_str = "  ".join(
                f"{s}:{c}" for s, c in sorted(p_dist.items(), key=lambda x: -x[1])
            )
            print(f"     The first {p_depth} word(s) matched {p_n} known rules ({dist_str})")
            print(f"     Matched prefix: {p_path}")
            if p_best:
                cb = _CSEV.get(p_best_sev, "")
                print(f"     → Closest known rule [{cb}{p_best_sev}{_RST}]: {_short(p_best)}")
        else:
            print(f"     The start of this rule has never been seen before.")

        # ── ② End-of-rule match ───────────────────────────────────────────
        c2 = _CSEV.get(s_sev, "")
        print(f"\n  ② END-OF-RULE MATCH    →  {c2}{s_sev or 'no match'}{_RST}  [{s_conf:.0%}]")
        if s_path_nat:
            dist_str = "  ".join(
                f"{s}:{c}" for s, c in sorted(s_dist.items(), key=lambda x: -x[1])
            )
            print(f"     The last {s_depth} word(s) matched {s_n} known rules ({dist_str})")
            print(f"     Matched suffix: ...{s_path_nat}")
            if s_best:
                cb = _CSEV.get(s_best_sev, "")
                print(f"     → Closest known rule [{cb}{s_best_sev}{_RST}]: {_short(s_best)}")
        else:
            print(f"     The end of this rule has never been seen before.")

        # ── ②b Exact suffix-chain match ──────────────────────────────────
        c2b = _CSEV.get(chain["sev"], "")
        print(f"\n  ②b EXACT SUFFIX CHAIN →  {c2b}{chain['sev'] or 'no match'}{_RST}  [{chain['conf']:.0%}]")
        if chain["all_matches"]:
            print("     Tried exact suffixes from longest to shortest:")
            for m in chain["all_matches"][:5]:
                dist_str = "  ".join(
                    f"{s}:{c}" for s, c in sorted(m["severity_distribution"].items(), key=lambda x: -x[1])
                )
                print(
                    f"     ...{m['match_suffix']}  ->  "
                    f"{m['n_master_matches']} known rule(s), {dist_str}"
                )
        else:
            print("     No exact full-suffix chain match was found in master rules.")

        # ── ③ Most-similar rules ──────────────────────────────────────────
        c3 = _CSEV.get(knn_sev, "")
        print(f"\n  ③ MOST SIMILAR RULES   →  {c3}{knn_sev}{_RST}  [{knn_conf:.0%}]")
        dist_str = "  ".join(f"{s}:{c}" for s, c in knn_ctr.most_common())
        sim_pct  = round(float(sims[0]) * 100)
        print(f"     Compared contiguous token chunks, not only individual words.")
        print(f"     Top {len(neigh_sevs)} most similar: {dist_str}")
        ct = _CSEV.get(neigh_sevs[0], "")
        print(f"     → Closest match ({sim_pct}% similar) [{ct}{neigh_sevs[0]}{_RST}]: {_short(neigh_rules[0])}")
...
def main():
    ...
    master["Severity"] = master["Severity"].astype(str).str.strip()
    ...
    print("=" * 70)
    explore_suffix(suf_root, "FREE-COOLING-MODE",
               min_rules=1, max_depth=4)
    ...
    explain_predictions(master, new_rules, pref_root, suf_root, top_k_neighbors=5)
